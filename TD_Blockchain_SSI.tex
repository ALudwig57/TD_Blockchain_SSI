...
En 1992, le protocole \say{ arbre de Merkle } fut utilisé pour combiner plusieurs documents dans un seul bloc. 
...
\section{Les tokens}
Toute blockchain publique fonctionne obligatoirement avec un jeton : \say{ le Token }. Il est programmable et personnalisable pour représenter ce qui sera échangé. Cela peut être une cryptomonnaie, un droit d’usage, un droit de paiement, une réputation… Cela peut prendre n’importe quelle forme et c’est ce qui sera utilisé et échangé en peer-to-peer sans duplication sur la blockchain.
...
Enfin il y a les consortiums, ce sont des blockchains hybrides.
    ...
    \begin{figure}[H]
        \begin{center}
          \includegraphics[width=.32\textwidth]{images/billard.jpg}
          \label{fig:billard}
    ...
    \\
    D’après Nestlé, cela permet de \say{ renforcer le lien de confiance avec ses consommateurs }. C’est pourquoi ces derniers appliquent cette solution sur 15 de leurs produits phares, notamment la purée Mousline. Carrefour compte quant à lui déployer cette solution d’ici 2020 sur l’ensemble des produits alimentaires Filière Qualité Carrefour, soit 300 références.
    \\
    ...
    Pour cette opération nous devons seulement être sûr de l’intégrité du \say{ Top Hash }, c’est pourquoi cette information sera rajoutée au contenu d’un bloc dont l'intégrité a été prouvée dans le point précédent.\cite{merkle}
    ...
    \begin{itemize}
        ...
        \item Créer / ouvrir des nouveaux blocs : Comme nous l’avons vu précédemment cette opération consiste à trouver un fichier \say{ nonce } afin que le hash du bloc respecte certaines règles et donc sécuriser la blockchain. Cela prend énormément de temps de calcul et par conséquent provoque un réel impact écologique (électricité).
        \newline
    \end{itemize}
    ...
    C'est la plus connue des vulnérabilités de la blockchain Ethereum de l'époque. Elle a supris tout le monde et a mené à un \emph{hard fork}\footnote{grosse modification}. La réentrance survient lorsque des appels de contrat externes sont autorisés à passer de nouveaux appels au contrat appelant avant la fin de l'exécution initiale. Pour une fonction, cela signifie que l'état du contrat peut changer en cours d'exécution à la suite d'un appel à un contrat non approuvé ou de l'utilisation d'une fonction de bas niveau avec une adresse externe.
    \\
    \\
    ...
    \begin{itemize}
        ...
        \item Quelqu'un sur le réseau voit la transactionb d'Alice (contenant la solution) en attente d'être minée et soumet cette même solution avec des frais plus élevés.
    ...
    \begin{itemize}
        ...
        \item Un peu avant minuit, le mineur finit par miner le bloc. L'heure actuelle est \say{assez proche de minuit}, les autres noeuds du réseau acceptent le bloc.
    \end{itemize}
    ...
    Le 15 avril 2019, l'ANSSI a publié son rapport d'activités de 2018, intitulé \say{Construire ensemble la confiance 
    numerique de demain}. 
