...
\end{itemize}
    ...
    \begin{figure}[H]
        \begin{center}
          \includegraphics[width=\textwidth]{images/attaque51.jpg}
          \label{fig:attaque51}
          \caption{Fonctionnement d'une attaque 51}
        \end{center}
    \end{figure}
    ...
    La blockchain est une technologie prometteuse respectant les principes de disponibilité et d'intégrité des données. Pour ce qui est de la confidentialité et de la traçabilité, il n'est pas possible de les respecter en même temps, l'une empêchant l'autre. Même de grands groupes s'y mettent comme IBM ou Nestlé. A partir du principe de cryptomonnaies, elle s'est imposée comme une technologie du futur, permettant le partage et la vérification des données sans tiers de confiance grâce au réseau peer-to-peer. La législation, quant à elle, peine à lui trouver un cadre juridique, certains allant jusqu'à l'interdire (L'Inde a interdit tout paiement par bitcoin). Elle n'est également pas tout à fait conforme au RGPD. Sans juridiction appropriée, cela peut sembler dangereux, comme le montre le libra : au début, beaucoup de personnes étaient enthousiastes de l'arrivée d'un gros dans ce domaine, mais récemment, beaucoup comment à prendre peur comme paypal, qui se desiste, ou des gouvernements, qui craignent un déreglement monétaire mondial~\cite{librapeur}.
    
    Nous sommes à l'aube d'une révolution, et toute révolution passe par trois étapes selon Shopenhauer :
    \begin{itemize}
        \item ridicule
        \item dangereux
        \item évident
    ...
    La blockchain fut considérée comme ridicule, utilisée seulement par les geeks, jusqu'à la flambée du bitcoin en décembre 2017, elle est maintenant dangereuse car elle a le potentiel de changer la façon dont le monde fonctionne.
